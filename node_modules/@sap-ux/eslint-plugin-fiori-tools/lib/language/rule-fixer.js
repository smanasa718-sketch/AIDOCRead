...
exports.createJsonFixer = createJsonFixer;
/**
 ...
 */
function createJsonFixer(config) {
    ...
    if (fixParent) {
        ...
        const parentObjectNode = context.sourceCode.getParent(node);
        // Parent member node (contains value)
        const parentMemberNode = parentObjectNode && context.sourceCode.getParent(parentObjectNode);
 ...
 */
function handleUpdate(fixer, node, value) {
    if (!node.value) {
        return undefined;
    }
 ...
 * @returns Fixer result or undefined if no fix is possible
 */
function handleInsert(fixer, node, missingSegments, value) {
    if (missingSegments.length === 0) {
        return undefined;
    }
 ...
 */
function handleDelete(fixer, node, context) {
 ...
 */
function findNodeIndex(parentMembers, startOffset, endOffset) {
    return parentMembers.findIndex((member) => member.name.loc.start.offset === startOffset && member.value.loc.end.offset === endOffset);
}
/**
 ...
 */
function findLineStart(sourceCode, startOffset) {
 ...
 */
function findDeleteEndOffset(sourceCode, endOffset, includeComma) {
 ...
 */
function findCommaOffset(sourceCode, previousValueEnd, startOffset) {
 ...
 */
function handleLastPropertyDelete(fixer, sourceCode, parentMembers, nodeIndex, endOffset, lineStartOffset) {
 ...
 */
function findParentObject(node, targetMember) {
    if (!node) {
        return undefined;
    }
 ...
 */
function isMemberMatchingTarget(member, targetMember) {
    return (member.name.loc.start.offset === targetMember.name.loc.start.offset &&
 ...
 * @returns The parent ObjectNode if found, undefined otherwise
 */
function searchInObjectMembers(node, targetMember) {
 ...
 * @returns The parent ObjectNode if found, undefined otherwise
 */
function searchInArrayElements(elements, targetMember) {
 ...
 */
function buildNestedJson(segments, value, currentIndent, indentSize) {
