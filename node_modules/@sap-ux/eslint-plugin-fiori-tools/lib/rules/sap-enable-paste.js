"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const diagnostics_1 = require("../language/diagnostics");
const rule_factory_1 = require("../language/rule-factory");
const rule_fixer_1 = require("../language/rule-fixer");
const rule = (0, rule_factory_1.createFioriRule)({
    ruleId: diagnostics_1.ENABLE_PASTE,
    meta: {
        type: 'suggestion',
        docs: {
            recommended: true,
            description: 'Sets whether to enable or disable the "Paste" button in tables',
            url: 'https://github.com/SAP/open-ux-tools/blob/main/packages/eslint-plugin-fiori-tools/docs/rules/sap-enable-paste.md'
        },
        messages: {
            [diagnostics_1.ENABLE_PASTE]: 'Paste functionality in the table must be enabled'
        },
        fixable: 'code'
    },
    check(context) {
        const problems = [];
        for (const [appKey, app] of Object.entries(context.sourceCode.projectContext.linkedModel.apps)) {
            const parsedApp = context.sourceCode.projectContext.index.apps[appKey];
            const parsedService = context.sourceCode.projectContext.getIndexedServiceForMainService(parsedApp);
            if (!parsedService) {
                continue;
            }
            if (app.type === 'fe-v4') {
                for (const page of app.pages) {
                    if (page.type !== 'object-page') {
                        continue;
                    }
                    problems.push(...handlePasteInTableV4(page, parsedApp));
                }
            }
        }
        return problems;
    },
    createJsonVisitorHandler: (context, diagnostic, deepestPathResult) => function report(node) {
        context.report({
            node,
            messageId: diagnostics_1.ENABLE_PASTE,
            fix: (0, rule_fixer_1.createJsonFixer)({
                context,
                deepestPathResult,
                node,
                operation: 'delete'
            })
        });
    }
});
/**
 * Looks through V4 app page tables and returns problems if enablePaste is set to false.
 *
 * @param page - V4 app page
 * @param parsedApp - parsed V4 app
 * @returns - EnablePaste issues
 */
function handlePasteInTableV4(page, parsedApp) {
    const problems = [];
    for (const table of page.lookup['table'] ?? []) {
        if (table.configuration.enablePaste.valueInFile === false) {
            problems.push({
                type: diagnostics_1.ENABLE_PASTE,
                pageName: page.targetName,
                manifest: {
                    uri: parsedApp.manifest.manifestUri,
                    object: parsedApp.manifestObject,
                    propertyPath: table.configuration.enablePaste.configurationPath
                }
            });
        }
    }
    return problems;
}
exports.default = rule;
//# sourceMappingURL=sap-enable-paste.js.map