...
class EdmJsonVisitor {
    ...
    [cds_annotation_parser_1.RECORD_TYPE] = (record, parent) => {
     ...
     * @param parent - The parent AnnotationNode.
     */
    convertElementName(property, element, parent) {
     ...
     * @param parent - The parent AnnotationNode.
     */
    convertCollection(items, element, parent) {
    ...
    [cds_annotation_parser_1.COLLECTION_TYPE] = () => {
        return undefined;
    };
    [cds_annotation_parser_1.EMPTY_VALUE_TYPE] = () => {
        return undefined;
    };
    [cds_annotation_parser_1.STRING_LITERAL_TYPE] = (node) => {
    ...
    [cds_annotation_parser_1.BOOLEAN_TYPE] = (node) => {
        let text = '';
        if (node.value === true) {
            text = 'true';
        }
    ...
    [cds_annotation_parser_1.NUMBER_LITERAL_TYPE] = (node, parent) => {
        const result = number_1.numberHandler.convert(this.state, node, parent);
