...
function getTokenRange(tokens, range) {
    const start = tokens.findIndex((0, cds_compiler_tokens_1.matchTokenByStart)(range.start));
    let end = -1;
    if (start > -1) {
        end = tokens
            .slice(start)
            .findIndex((token) => (0, cds_compiler_tokens_1.tokenLine)(token) > range.end.line ||
            ((0, cds_compiler_tokens_1.tokenLine)(token) === range.end.line && (0, cds_compiler_tokens_1.tokenColumn)(token) >= range.end.character));
        end = start + end - 1;
    }
    return { start, end };
}
