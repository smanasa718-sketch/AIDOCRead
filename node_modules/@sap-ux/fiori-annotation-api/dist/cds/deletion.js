...
function canIncludePrecedingComment(tokens, options) {
    ...
    return !precedingNonCommentToken || (0, cds_compiler_tokens_1.tokenLine)(precedingNonCommentToken) < (0, cds_compiler_tokens_1.tokenLine)(commentToken);
}
/**
...
function canIncludeSubsequentComment(deletionRange, tokens, options) {
    ...
    if ((0, cds_compiler_tokens_1.tokenLine)(commentToken) > (0, cds_compiler_tokens_1.tokenLine)(tokens[deletionRange.tokenRange.end])) {
        return false;
    } // comment not in same line
    const nextToken = getNeighboringToken(tokens, afterTokenIndex, +1);
    return !nextToken || (0, cds_compiler_tokens_1.tokenLine)(nextToken) > (0, cds_compiler_tokens_1.tokenLine)(commentToken); // make sure no further non-comment token after comment
}
/**
...
function getPositionFromToken(token, endPosition = false) {
    ...
    return odata_annotation_core_types_1.Position.create((0, cds_compiler_tokens_1.tokenLine)(token), (0, cds_compiler_tokens_1.tokenColumn)(token) + offset);
}
...
function includeSiblingSeparator(tokens, deletionRange) {
    ...
    if (nextToken?.text === separator) {
        ...
        if (commentToken?.text.startsWith('//')) {
            if ((0, cds_compiler_tokens_1.tokenLine)(tokens[deletionRange.tokenRange.end]) === (0, cds_compiler_tokens_1.tokenLine)(commentToken)) {
