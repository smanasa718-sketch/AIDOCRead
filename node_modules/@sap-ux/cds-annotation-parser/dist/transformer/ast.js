...
class CstToAstVisitor extends factory_1.Visitor {
     ...
     */
    buildOperator(operatorToken, range) {
        return { type: annotation_ast_nodes_1.OPERATOR_TYPE, value: operatorToken.image, range };
    }
    /**
    ...
    path(context, location) {
        ...
        const separators = [
            ...(context.pathSegment ?? []).flatMap((segment) => {
                const quotedIdentifiers = segment.children.quotedIdentifier?.flatMap((quotedIdentifier) => {
                ...
                const delimitedIdentifiers = segment.children.delimitedIdentifier?.flatMap((delimitedIdentifier) => {
                    if (!delimitedIdentifier.children.DelimitedIdentifier) {
                        return [
                            {
                                token: this.createEmptyIdentifier(delimitedIdentifier.children.IdentifierStart[0], delimitedIdentifier.children.DelimitedIdentifierExit[0]),
    ...
    struct(context, location) {
        ...
        const { properties: allProperties, annotations: allAnnotations } = (context.assignment ?? [])
            ...
            else if (assignment.children?.value?.length &&
