...
class AnnotationParser extends chevrotain_1.CstParser {
    ...
    createOrSubruleEntry = (rule, options) => ({
        ALT: () => {
            this.SUBRULE(rule, options);
        }
    });
    createOrConsumeEntry = (token, gate) => ({
        ALT: () => {
            this.CONSUME(token);
        },
        GATE: gate
    ...
    [PATH_TYPE] = this.RULE(PATH_TYPE, (valuePath) => {
    ...
    [PATH_SEGMENT_TYPE] = this.RULE(PATH_SEGMENT_TYPE, (valuePath) => {
        this.OR({
            DEF: [
