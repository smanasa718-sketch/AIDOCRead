...
exports.toPropagatedTargetMap = void 0;
...
const LOCALIZED_ENTITY = 'localized-entity';
...
/**
 * Recursively checks if given artifact has an origin as localized entity in its origin chain
 * @param targetArtifact - target artifact to check
 * @returns - true if some of the given artifact origins is localized entity
 */
function hasOriginAsLocalizedEntity(artifact) {
    if (!artifact._origin) {
        return false;
    }
    if (artifact._origin['$inferred'] === LOCALIZED_ENTITY) {
        return true;
    }
    else {
        return hasOriginAsLocalizedEntity(artifact._origin);
    }
}
function buildMapForTarget(targetMaps, ast, targetName, targetPathSegments, targetDefinition, isPropagatedElement = false) {
    ...
    if (targetMaps.visitedTargetMap[fullTargetName]) {
        return;
    }
    ...
    if (finalType) {
        ...
        if (targetArtifact &&
            ['entity', 'view'].includes(targetArtifact.kind) &&
            targetArtifact.name &&
            !(targetArtifact.$inferred === LOCALIZED_ENTITY || hasOriginAsLocalizedEntity(targetArtifact))) {
...
function buildGhostFiles(ast, files) {
    ...
    for (const relativeUri of sourceUris.values()) {
        ...
        const sourceFile = filesMatchingRelativePath.toSorted((a, b) => a.uri.length - b.uri.length)[0];
        if (!sourceFile) {
            continue;
        }
        const sourceTargets = sourceFile.targets;
        const ghostTargets = [];
        for (const target of sourceTargets) {
            const propagatedTargets = propagationMap[target.name] ?? {};
            for (const propagatedTargetName of Object.keys(propagatedTargets)) {
                const ghostTarget = structuredClone(target);
                ghostTarget.name = propagatedTargetName;
                ghostTargets.push(ghostTarget);
            }
        }
        const file = structuredClone(sourceFile);
        file.targets = ghostTargets;
        file.uri = odata_annotation_core_1.GHOST_FILENAME_PREFIX + file.uri;
        ghostFiles[sourceFile.uri] = file;
    }
    const result = files
        .map((file) => ghostFiles[file.uri])
        .filter((file) => !!file);
    return result;
}
