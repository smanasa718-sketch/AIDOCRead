...
function collectDraftEnabledProperty(context, parentInfo, propSegment) {
    ...
    switch (propSegment.toUpperCase()) {
        case types_1.IS_ACTIVE_ENTITY_ELEM_NAME.toUpperCase(): {
            const elementName = types_1.IS_ACTIVE_ENTITY_ELEM_NAME;
            const mdElementName = parentKey + '/' + elementName;
            if (parentXsnNode) {
                const draftPropertyNode = converter.convertDraftPropertyNode(parentXsnNode, elementName, 'Edm.Boolean', parentInfo.parentKey);
        ...
        case types_1.DRAFT_MESSAGES_ELEMENT_NAME.toUpperCase(): {
            if (parentInfo.parentXsnNode?.kind !== 'entity' && parentInfo.parentXsnNode?.kind !== 'view') {
                break;
            }
            const serviceName = converter.getNameObj(parentInfo.parentXsnNode._service)?.id;
            if (!context.compileModel.options.beta?.draftMessages || !serviceName) {
                break;
                return parentKey + '/' + types_1.HAS_ACTIVE_ENTITY_ELEM_NAME; // draft messages are not enabled
            }
            const elementName = types_1.DRAFT_MESSAGES_ELEMENT_NAME;
            const mdElementName = parentKey + '/' + elementName;
            if (parentXsnNode) {
                const draftPropertyNode = converter.convertDraftPropertyNode(parentXsnNode, elementName, `Collection(${serviceName}.DRAFT_DraftAdministrativeData_DraftMessage)`, parentInfo.parentKey);
                parentNode.content.push(draftPropertyNode);
                metadataElementMap.set(parentKey + '/' + elementName, { node: draftPropertyNode, parentKey });
            }
            return mdElementName;
        }
        case types_1.HAS_ACTIVE_ENTITY_ELEM_NAME.toUpperCase(): {
            const elementName = types_1.HAS_ACTIVE_ENTITY_ELEM_NAME;
            const mdElementName = parentKey + '/' + elementName;
            if (parentXsnNode) {
                const draftPropertyNode = converter.convertDraftPropertyNode(parentXsnNode, elementName, 'Edm.Boolean', parentInfo.parentKey);
                parentNode.content.push(draftPropertyNode);
    ...
    // defaults to 'HasActiveEntity', this is not entirely correct, but it has been working like this and changing it would not be easy
    return parentKey + '/' + types_1.HAS_ACTIVE_ENTITY_ELEM_NAME;
}
...
function isDraftEnabledProperty(context, propSegment, parentXsnNode) {
    ...
    return ((types_1.IS_ACTIVE_ENTITY_ELEM_NAME.toUpperCase() === propSegmentUc ||
        ...
        types_1.DRAFT_MESSAGES_ELEMENT_NAME.toUpperCase() === propSegmentUc) &&
        !!parentXsnNode &&
