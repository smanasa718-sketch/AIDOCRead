...
function convertTokens(tokens) {
    ...
    for (let index = 0; index < tokens.length; index++) {
        ...
        if ((0, tokenStream_1.isOldToken)(token)) {
            const previousToken = tokens[index - 1];
            if (previousToken &&
                (0, tokenStream_1.isOldToken)(previousToken) &&
        ...
        output.push(convertToken(token, index - tokenIndexOffset));
    }
    return output;
}
function convertToken(token, index) {
    const text = token.text === '<EOF>' ? '' : token.text;
    const { line, column, endLine, endColumn } = (0, tokenStream_1.getTokenLocation)(token, text);
    return {
        text,
        line: line - 1,
        character: column - 1,
        endLine: endLine - 1,
        endCharacter: endColumn - 1,
        tokenIndex: index,
        isComment: () => text.startsWith('//') || text.startsWith('/*')
    };
}
